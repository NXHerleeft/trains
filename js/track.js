...
class Track {
    ...
    _linkTrack() {
        if(typeof this.next === 'string')
            this.next = this.trackLayout.getTrack(this.next);
        if(typeof this.previous === 'string')
            this.previous = this.trackLayout.getTrack(this.previous);
    }
    ...
    _linkSubTracks() {
        ...
        this.subs.forEach(function(sub, index) {
            if(typeof sub.previous === 'string') { // If a previous track was set manually
                sub.previous = this.trackLayout.getSubTrack(sub.previous);
            } else {
                if (index === 0) { // It's the first one in the list
                    if (this.previous !== null)
                        sub.previous = this.previous.getSubTrackAt(sub.x0, sub.y0, 1);
                } else {
                    sub.previous = prev;
                }
            }

            if(typeof sub.next === 'string') { // If the next track was set manually
                sub.next = this.trackLayout.getSubTrack(sub.next);
            } else {
                if (index === nrSubs - 1) { // It's the last one in the list
                    if (this.next !== null)
                        sub.next = this.next.getSubTrackAt(sub.x1, sub.y1, 1);
                } else {
...
class SwitchTrack extends Track {
    ...
    _linkTrack() {
        if(typeof this.nextNormal !== 'undefined')
            this.nextNormal = this.trackLayout.getTrack(this.nextNormal);
        if(typeof this.nextReversed !== 'undefined')
            this.nextReversed = this.trackLayout.getTrack(this.nextReversed);
...
class StraightSubTrack extends NormalSubTrack {
     ...
     */
    isAt(x, y, maxDrift) {
        if(pointsToDistance(this.x0, this.y0, x, y) <= maxDrift)
            return true;
        else if(pointsToDistance(this.x1, this.y1, x, y) <= maxDrift)
            return true;
